    ...
    <div class="center_content">
      ...
      <content>
        <div
          ...
          :style="{
            backgroundImage:
              'url(' + require('../' + ProductTypeBanner.backgroundimg) + ')',
          }"
        >
          <div class="banner_wordbox">
            <h3>
              {{ ProductTypeBanner.typetitle }}
              <img
                v-if="ProductTypeBanner.typeimg != ''"
                :src="require('../' + ProductTypeBanner.typeimg)"
                ...
                <div class="card_bottom_price_and_icon">
                  ...
                  <div class="card_bottom_icon">
                    <Icon
                      ...
                      width="25"
                      class="hearticon"
                      @click.native.stop="heartIconToggle(i)"
                      v-if="ProductInCartId.indexOf(item.id) == -1"
                    ></Icon>
                    <Icon
                      inline
                      icon="ph:heart-fill"
                    ...
                    <Icon
                      ...
                      width="25"
                      @click.native.stop="cartIconToggle(i)"
                      class="carticon"
                    ></Icon>
                  </div>
                </div>
              ...
              <div class="product_singlepage_box">
                ...
                <div class="product_singlepage_content">
                  ...
                  <div class="product_singlepage_info">
                    ...
                    <button @click="heartIconToggle(i)">收藏筆刷</button>
                  </div>
                </div>
              </div>
            </div>
...
export default {
    ...
    heartIconToggle(i) {
      if (
        this.ProductInCartId.findIndex(
          (item) => item == this.ProductType[i].id
        ) == -1
      ) {
        this.isActive = true;
        setTimeout(() => (this.isActive = false), 1000);
        this.ProductInCartInfo.push(this.ProductType[i]);
        this.ProductInCartId.push(this.ProductType[i].id);
        this.saveCartInfo();
      } else if (
        this.ProductInCartId.findIndex(
          (item) => item == this.ProductType[i].id
  ...
  computed: {
    ProductType() {
      if (this.RouterType == "freebrush") {
        return this.ProductDataJson.products.filter(
          (p) => p.ProductPrice === 0
        );
      } else if (this.RouterType == "paybrush") {
        return this.ProductDataJson.products.filter((p) => {
          return p.ProductPrice !== 0 && p.ProductType !== "paper";
        });
      } else {
        return this.ProductDataJson.products.filter(
          (p) => p.ProductType === this.RouterType
        );
      }
    },
    heartIcon() {
...
<style lang="scss" src="../assets/css/a_brushcate.scss" scoped></style>
